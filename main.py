...
def verify_files():
    for p, d in [
        ...
        (DESCRIPTOR_MODEL, "MARS descriptor")
    ]:
...
def main():
    ...
    while True:
        ...
        for (x,y,w_,h_), conf, f_cnn in zip(bboxes, scores, cnn_feats):
            ...
            print(f_cnn)
            print(hist)
            print(len(f_cnn),len(hist),len(combined))
        ...
        if key == ord('q'):
            ...
            # Lưu frame hiện tại
            filename = f"bc/frame_"
            # cv2.imwrite(filename+f"{frame_idx:06d}_pre.png", frame)
            cv2.imwrite(filename + f"{frame_idx:06d}_nor.png", frame_nor)
            # i=0
            # for (x,y,w_,h_), conf, f_cnn in zip(bboxes, scores, cnn_feats):
            #     crop = frame_nor[y:y+h_, x:x+w_]
            #
            #     cv2.imwrite(filename + f"{i:06d}_face.png", crop)
            #     i+=1
            print(f"[INFO] Saved")
    # Cleanup
    cap.release()
